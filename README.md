# Teste

## Competências
- Introdução à Computação
- Lógica de Programação
- Algoritmo e Lógica de Programação - Linguagem C

## Habilidades
- Desenvolvimento de jogo pela plataforma RayLib em Linguagem C
- links para repositório
- CISCO Network

## Projetos Acadêmicos
- Desenvolvimento de jogo pela plataforma RayLib em Linguagem C
- 

## Atitudes
- Organização
- Proatividade
- Liderança
- Trabalho em Equipe

## Objetivos
- Frontend
- Backend
- C++

## Onde você quer trabalhar?
- Desvolvimento de Software


# Cecilia Alemi
Estudante de **Engenharia de Software no IDP (Brasília)**, atualmente no 1º semestre.

### Competências
- Lógica Computacional.
- Algoritmos e Lógica de Programação.
- Introdução a Computação.

### Habilidades
- **Linguagens:** C (Foco em lógica e gestão de memória).
- **Ambiente de Dev:** Visual Studio Code (VS Code) e NotePad++.
- **Versionamento:** GitHub.

### Atitudes
- Organizada.
- Trabalho em equipe.
- Liderança.

### Objetivo
- Desenvolver minhas competências e habilidades em Engenharia de Software, buscando trabalhar na área de Análise de Dados.
  
### Projetos Acadêmicos
- **Criptografia e Segurança:** Desenvolvendo um artigo técnico em grupo sobre como as **bijeções matemáticas** e como elas garantem a confidencialidade e integridade no meio digital.
- **Jogo com Impacto Social** Desenvolvi em conjunto com outros 6 alunos um jogo de cunho educacional através da plataforma RayLib em **linguagem C** com o intuito de promover a conscientização sobre a importância dos bombeiros, tivemos como público-alvo crianças de até 10 anos.

### Metas
- Curso durante as férias de inverno para aprofundamento da linguagem C
- Curso introdutório de JavaScript
- BootCamp Santander para aprendizado FullStack
- Aprofundar meus conhecimentos em **Estrutura de Dados**.
- Explorar os sistemas de automação aplicado em relatórios e a estética de dados.
- Buscando minha primeira oportunidade de **estágio** na área de Dados ou Desenvolvimento.

### Como me encontrar:
- **LinkedIn:** [linkedin.com/in/cecilialemi](https://www.linkedin.com/in/cecilialemi/)
- **E-mail:** cecili.alemis@gmail.com


